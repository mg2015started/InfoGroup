...
def cal_info_gain_ratio(dataset, feature_name, label_name):
    """
    cal the info gain ratio gr(dataset, feature)
    @param dataset: DataFrame
    @param feature: string
    @return: info gain ratio: float
    """
    ...
    groups = dataset.groupby(feature_name)
    probs = groups.size() / dataset_size
    dataset_entropy_related_feature = - np.sum(probs * np.log2(probs))

    # cal gr(D, A)
    info_gain_ratio = info_gain / dataset_entropy_related_feature

    print(feature_name, info_gain_ratio, dataset_entropy_related_feature, info_gain, dataset_entropy,
          dataset_condition_entropy)
...
def choose_best_feature(dataset, label_name):
    ...
    for feat in features:
        info_gain = cal_info_gain_ratio(dataset, feat, label_name)
